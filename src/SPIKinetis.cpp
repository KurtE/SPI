...
bool SPIClass::transfer16(const uint16_t *buf, uint16_t *retbuf, size_t count, void(*callback)(void)) {
	...
	if (buf) {
		...
		_dmaTX->transferCount((count-1));	// This one is not divided by minor loop
	}	
	if (retbuf) {
		// On T3.5 must handle SPI1/2 differently as only one DMA channel
		...
		_dmaRX->transferCount(count);	// This one is not biased by size of minor loop
	}
...
//			_dmaRX->transferCount(hardware.max_dma_count);
			...
			_dmaTX->transferCount(_dma_count_remaining-1);
...
void SPIClass::enable_pins(void) {
	...
	mux = PORT_PCR_MUX(hardware.mosi_mux[mosi_pin_index]);
	*reg = mux;

	// MISO
	reg = portConfigRegister(hardware.miso_pin[miso_pin_index]);
	mux = PORT_PCR_MUX(hardware.miso_mux[miso_pin_index]);
	*reg = mux;

	// SCK
	reg = portConfigRegister(hardware.sck_pin[sck_pin_index]);
	mux = PORT_PCR_MUX(hardware.sck_mux[sck_pin_index]);
	*reg = mux;
}

void SPIClass::disable_pins(void)  {
	//serial_print("disable_pins\n");
	volatile uint32_t *reg;

	// Warning: Unlike the older version, this one simply
	// disables the pin instead of trying to make it an IO pin
...
SPIClass SPI(KINETISK_SPI0, SPIClass::spi0_hardware);
#endif
...
#if SPI_INTERFACES_COUNT > 0
const SPIClass::SPI_Hardware_t SPIClass::spi0_hardware = {
	SIM_SCGC6, SIM_SCGC6_SPI0, 4, IRQ_SPI0, 32767, 
	DMAMUX_SOURCE_SPI0_TX, DMAMUX_SOURCE_SPI0_RX, 
	_spi_dma_rxISR0,
	12, 8, 39, 255, 
	2, 2, 2, 0, 
	11, 7, 28, 255,
...
SPIClass SPI1(KINETISK_SPI1, SPIClass::spi1_hardware);
#endif
...
SPIClass SPI2(KINETISK_SPI2, SPIClass::spi2_hardware);
#endif
...
void SPIClass::begin()
{
	...
	enable_pins(); // pins managed by SPCRemulation in avr_emulation.h
}
...
SPIClass SPI(KINETISL_SPI0, SPIClass::spi0_hardware);
#endif
...
SPIClass SPI1(KINETISL_SPI1, SPIClass::spi1_hardware);
#endif
